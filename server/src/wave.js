...
export function clampParams(raw = {}) {
  ...
  const sizeMm = clamp(num(raw.sizeMm, base.sizeMm), 40, 280);
  const maxOffset = sizeMm * 0.5;
  const harmonics = defaultHarmonics().map((h, i) => {
    const src = raw.harmonics?.[i] ?? {};
    return {
      amplitude: clamp(num(src.amplitude, h.amplitude), 0, 2),
      wavelength: clamp(num(src.wavelength, h.wavelength), 2, 120),
      angle: clamp(num(src.angle, h.angle), 0, 360),
      phase: clamp(num(src.phase, h.phase), 0, Math.PI * 2),
      offset: clamp(num(src.offset, h.offset), 0, maxOffset),
    };
  });

  return {
    name: String(raw.name ?? base.name).slice(0, 80),
    harmonicCount: Math.round(clamp(num(raw.harmonicCount, base.harmonicCount), 1, 6)),
    harmonics,
    wavelengthScale: clamp(num(raw.wavelengthScale, base.wavelengthScale), 0.25, 3),
    amplitudeScale: clamp(num(raw.amplitudeScale, base.amplitudeScale), 0.2, 2.5),
    sizeMm,
    waveHeightMm: clamp(num(raw.waveHeightMm, base.waveHeightMm), 0.6, 32),
    baseThicknessMm: clamp(num(raw.baseThicknessMm, base.baseThicknessMm), 0.8, 10),
    falloff: clamp(num(raw.falloff, base.falloff), 0, 1),
    resolution: Math.round(clamp(num(raw.resolution, base.resolution), 24, 180)),
...
export function meshStats(params) {
  ...
  const bottom = top;
  const walls = (n - 1) * 8;
  const triangles = top + bottom + walls;
